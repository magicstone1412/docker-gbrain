...
- Embedding provider priority is hard-coded in `entrypoint.sh`: `VOYAGE_API_KEY` → `OPENAI_API_KEY` → `--no-embedding`.
- Voyage uses the persisted config pair `voyage:voyage-4` plus `1024` dimensions; the entrypoint patches `~/.gbrain/config.json` after `gbrain init` because init overwrites it.
